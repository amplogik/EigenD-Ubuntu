...
class GarbageCollector(threading.Thread):
    ...
    def passes(self):
        ...
        while True:
            for i in range(0, 10):
                yield 0  # Young generation
            for i in range(0, 10):
                yield 1  # Middle generation
...
class Backend(eigend_native.c2p):
    ...
    def __create_context(self, name):
        if self.frontend:
            ...
            return self.scaffold.bgcontext(
                piw.tsd_scope(), utils.statusify(None), logger, name
            )
        else:
            return None
    ...
    def save_setup(self, slot, tag, desc, make_default):
        ...
        def not_done(*args, **kwds):
            ...
            self.info_dialog(
                "Setup Not Saved",
                "Setup Not Saved",
                "The user setup '" + slot + "' could not be saved\n" + args[1],
            )
        ...
        def done(*args, **kwds):
            ...
            self.info_dialog(
                "Setup Saved",
                "Setup Saved",
                "The user setup '" + slot + "' was successfully saved",
            )

        r = self.run_background(
            self.agent.save_file, self.save_tmp(filename, "~"), desc
        )
    ...
    def __get_email(self):
        filename = resource.user_resource_file(
            resource.global_dir, resource.user_details, version=""
        )
        try:
    ...
    def file_bug(self, user, email, subj, desc):
        ...
        resource.file_open(filename, "w").write(
