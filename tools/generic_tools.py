...
class PiGenericEnvironment(SCons.Environment.Environment):
    ...
    def Finalise(self):
        ...
        print(
            "building release",
            self.subst("$PI_RELEASE"),
            "compatible with",
            self.subst("$PI_COMPATIBLE"),
        )
    ...
    def __getpython(self):
        status = os.popen(
            '"%s" %s'
            % (self["PI_PYTHON"], join(os.path.dirname(__file__), "detect.py")),
            "r",
        ).read(1024)
        (exe, incpath, libpath, libs, linkextra, prefix) = [
    ...
    def __init__(self, platform, install_prefix, userdir_suffix, python=None, **kwds):
        ...
        self.Replace(
            RELEASESTAGEDIR=join("$RELEASESTAGEROOTDIR", "$PI_COLLECTION-$PI_RELEASE")
        )
        ...
        self.Replace(
            MODSTAGEDIR_GLOBAL=join("$RELEASESTAGEDIR", "modules", "$PI_PYTHONPKG")
        )
        self.Replace(MODRUNDIR_GLOBAL=join("#tmp", "modules", "$PI_PYTHONPKG"))
        self.Replace(
            MODSTAGEDIR_PLUGIN=join(
                "$RELEASESTAGEDIR", "plugins", "$PI_ORGANISATION", "$PI_AGENTGROUP"
            )
        )
        self.Replace(
            MODRUNDIR_PLUGIN=join(
                "#tmp", "plugins", "$PI_ORGANISATION", "$PI_AGENTGROUP"
            )
        )
        ...
        self.Replace(
            BINSTAGEDIR_PLUGIN=join(
                "$RELEASESTAGEDIR", "plugins", "$PI_ORGANISATION", "$PI_AGENTGROUP"
            )
        )
        self.Replace(
            BINRUNDIR_PLUGIN=join(
                "#tmp", "plugins", "$PI_ORGANISATION", "$PI_AGENTGROUP"
            )
        )
        ...
        self.Replace(
            PYDSTAGEDIR_PLUGIN=join(
                "$RELEASESTAGEDIR", "plugins", "$PI_ORGANISATION", "$PI_AGENTGROUP"
            )
        )
        self.Replace(
            PYDRUNDIR_PLUGIN=join(
                "#tmp", "plugins", "$PI_ORGANISATION", "$PI_AGENTGROUP"
            )
        )
        ...
        self.Replace(
            PI_RELEASE=lambda target, source, env, for_signature: self.shared.release
        )
        self.Replace(
            PI_COMPATIBLE=lambda target,
            source,
            env,
            for_signature: self.shared.compatible
        )
        self.Replace(
            PI_COLLECTION=lambda target,
            source,
            env,
            for_signature: self.shared.collection
        )
        self.Replace(
            PI_ORGANISATION=lambda target,
            source,
            env,
            for_signature: self.shared.organisation
        )
        ...
        self.Replace(
            PI_PIPCMD=os.path.join(os.path.dirname(__file__), "pip_cmd", "pip.py")
        )
    ...
    def PiLexicon(self, target, source, package=None, per_agent=None):
        return self.PiDynamicPython(
            target,
            source,
            PiGenericEnvironment.build_lexicon,
            package=package,
            per_agent=per_agent,
        )
    ...
    def PiPythonPackage(self, package=None, per_agent=None, subdirs=(), resources=()):
        ...
        node = env.PiDynamicPython(
            "version.py", [], build_version, package=package, per_agent=per_agent
        )
    ...
    def PiBranding(
        ...
        if bmp and not self.get("PI_BMPLOGO"):
            self.Replace(PI_BMPLOGO=self.File(bmp).srcnode().abspath)
        if ico and not self.get("PI_ICOLOGO"):
            self.Replace(PI_ICOLOGO=self.File(ico).srcnode().abspath)
        if icn and not self.get("PI_ICNLOGO"):
            self.Replace(PI_ICNLOGO=self.File(icn).srcnode().abspath)
        if cert and not self.get("PI_CERTFILE"):
            self.Replace(PI_CERTFILE=self.File(cert).srcnode().abspath)
        if bg and not self.get("PI_BACKGROUND"):
            self.Replace(PI_BACKGROUND=self.File(bg).srcnode().abspath)
        if license and not self.get("PI_LICENSE"):
            self.Replace(PI_LICENSE=self.File(license).srcnode().abspath)
        if passwd and not self.get("PI_CERTPASS"):
            self.Replace(PI_CERTPASS=passwd)
    ...
    def PiPipBinding(
        ...
        cppnode = self.Command(
            cppfile,
            spec,
            '"$PI_PYTHON" "$PI_PIPCMD" "' + module + '" $SOURCES $TARGET ' + inc,
        )
        ...
        binding = bind_env.SharedLibrary(
            module,
            allsources,
            SHLIBPREFIX=bind_env["PI_MODPREFIX"],
            SHLIBSUFFIX=bind_env["PI_MODSUFFIX"],
            SHLINKFLAGS=bind_env["PI_MODLINKFLAGS"],
        )
        ...
        if package:
            ...
            binding_tgt2 = bind_env.Install(
                bind_env.subst("$PYDSTAGEDIR"), binding_tgt1
            )
    ...
    def __build_manifest(self, module, package, agent_list, manifest):
        ...
        for a in agent_list:
            ...
            if extra:
                extra = ":" + extra
            textnodes.append(
                env.Value("agent %s:%s:%s:%s%s" % (a[0], a[1], a[2], a[3], extra))
            )
            vnode = env.Command(
                join(env.subst("$MODRUNDIR_PLUGIN"), "%s_version.py" % a[0]),
                [env.Value(a[0]), env.Value(a[2])],
                build_version,
            )
            if package:
        ...
        mnode = env.Command(
            join(env.subst("$MODRUNDIR_PLUGIN"), "Manifest"), textnodes, build_manifest
        )
    ...
    def PiPythonWrapper(self, name, pypackage, module, main, package=None, gui=False):
        ...
        run_source_node = self.baker(
            run_source, self.py_template, main=main, package=pypackage, module=module
        )
    ...
    def PiPythonwWrapper(
        ...
        return self.PiPythonWrapper(
            name, pypackage, module, main, package=package, gui=True
        )
    ...
    def PiGuiProgram(
        ...
        return self.PiProgram(
            target, sources, libraries=libraries, package=package, hidden=hidden
        )

    def PiProgram(
        self, name, sources, libraries=[], package=None, gui=False, hidden=True
    ):
        env = self.Clone()
        env.Append(PILIBS=libraries)
...
extern int main(int argc, char **argv)
{
...

  {
      ...
      {
          char tmpbuffer[4096];
          init_release_root(tmpbuffer);

          const char *p = tmpbuffer;
          while(*p)
          {
              if(*p=='\\\\') escbuffer += "\\\\\\\\";
              else escbuffer += *p;
      ...
      cmdbuffer += "from %(package)s import %(module)s\\n";
